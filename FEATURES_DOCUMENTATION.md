...
| `dns_answer_ttls_max`| **Derived**| Max(TTLs) | Fast Flux / Evasion | Detects if *any* record has unusually high caching. |
| `dns_answer_ttls_min`| **Derived**| Min(TTLs) | Fast Flux | Detects if *any* record has very low caching. |
| `dns_total_queries` | **Derived** | Count(QR=0) | **DDoS (Flood)** | Total volume of questions sent. |
| `dns_total_responses`| **Derived** | Count(QR=1) | Reflection | Total volume of answers received. |
...
| `dns_edns_udp_size`| **Direct** | OPT Payload Field | Amplification | Requesting 4096+ byte buffers indicates intent for huge replies. |
