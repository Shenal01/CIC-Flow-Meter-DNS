"""
Test Script for Random Forest DNS Infrastructure Attack Detection Model

This script loads a trained Random Forest model and tests it on a provided dataset.
...
# Model file path
MODEL_FILE = 'random_forest_dns_memory_efficient.pkl' 
...
OUTPUT_FILE = 'randomforest_test_predictions.csv'
...
if 'protocol' in df_test_clean.columns:
    if df_test_clean['protocol'].dtype == 'object':
        ...
        # Custom mapping to ensure consistency with training (TCP=0, UDP=1)
...
y_prob = model.predict_proba(X_test)[:, 1]  # Probability of attack class
